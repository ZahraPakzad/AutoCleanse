...
class ClassifierDummy:
    ...
    def compute_scores(self, X, y):
        scores = cross_val_score(
            self.model, X, y, scoring='accuracy', cv=self.cv, n_jobs=-1)
...
class ClsNNBase(nn.Module):
    ...
    def add_regularization_hook(self, module, input, output):
        ...
        l2_reg = self.l2_strength * \
            F.mse_loss(output, torch.zeros_like(output))
    ...
    def train_model(self, num_epochs, batch_size, patience, layers, train_loader, val_loader,
        ...
        for epoch in range(num_epochs):
            train_progress = tqdm(
                train_loader, desc=f'Epoch [{epoch+1}/{num_epochs}], Training Progress', position=0, leave=True)
            ...
            # Final loss: multiply by batch size then averaged over all samples
            average_loss = running_loss / running_sample_count
            train_accuracy = accuracy_score(train_targets, train_predictions)
            train_precision = precision_score(
                train_targets, train_predictions, average="macro", zero_division=0)
            ...
            val_progress = tqdm(
                val_loader, desc=f'Epoch [{epoch+1}/{num_epochs}], Validation Progress', position=0, leave=True)
            ...
            scheduler.step()
            print(
                f"Epoch [{epoch+1}/{num_epochs}]: Learning Rate = {scheduler.get_last_lr()}\n")
    ...
    def save(self, location, name=None):
        ...
        if (name is None):
            layers_str = '_'.join(str(item)
                                  for item in layers)  # @TODO: file name hack
    ...
    def load(self, location, name=None):
        ...
        if (location == "bucketfs"):
            try:
                ...
                raise RuntimeError(
                    f"Failed loading {name} from BucketFS") from e
    ...
    def test(self, test_loader, batch_size, device):
        self.eval()
        test_progress = tqdm(
            test_loader, desc=f'Test Progress', position=0, leave=True)
